...
public:
  ...
  int *check(int i, int j, int ijValue, int dir, int *matrix[4]) {
    ...
    //right
		int rightCount = 0;                            //列check 
    if (j + 1 < N && dir != left) {
			for (int k = j + 1; k < N; k ++) {
				if (ijValue == matrix[i][k]) {
					rightCount ++;                         //行往右check统计有几个指相同的对象 
				} else {
					break;		
				}
			}
    ...
    if ( j - 1 > -1 && dir != right) {
		...
		//leftCount + rightCount >= 3                 //如果左右check发现相同值>=2 
		if (rightCount + leftCount >= 2) 
		{
			result[0] = 1;                              //reslut数组首元素置为1 
			if (rightCount) {                                   //如果有值相同的2个对象的话 result[1]也是 result的第二个值为相同对象个数的值 
				result[right] = rightCount;
			} 
			if (leftCount) {                       //对于left  result[2]的值置为往左chek时有几个相同对象个数的值 
        result[left] = leftCount;
			}
		}
    //up
		int upCount = 0;                                   //行check 
    if (i - 1 > -1 && dir != down) {
			for (int k = i - 1; k > -1; k --) {
      ...
      for (int j = 0; j < N; j++) {
        ...
        if (matrix[i][j] > 0) {
          ...
          if (i == M - 1)
		   ...
		   {      //不是最后一个行 
						u = matrix[i + 1][j] == -1 ? -2 : matrix[i + 1][j];  //判断是否为以消除的对象 ,是返回-2 不看了 否则获取值继续 
            downSwapResult =   check(i + 1 , j, d, down, matrix);                      //继续往下check 
          }
          if (j == N - 1)  //判断是否为最后一个列 
		  {                  
            l = -2;
		  ...
		  {                     //不是下一列，判断是否是已消除的 不是获取下一个对象的值 
						l = matrix[i][j + 1] == -1 ? -2 : matrix[i][j + 1];
          }
				//然后这些事中间部分 左右上的检查check 
...
int main() {
  ...
  cout<<(xxl.K)<<endl;
  	...
  	cout<<"所消耗时间:"<<double(finish - start)<<"ms"<<endl;
